		...
		<div id="paper">
			...
			<div id="company_profile">
				...
				<div id="company_info">
					...
					<ul id="page_move">
						<!--페이지 이동-->
						<li class="on"><a href="news?code=${k_homeVO.companyCode}">News</a></li>
							...
							<c:forEach items="${IndividualVO}" var="IndividualVO">
								...
								<ul class="title_box">
									<c:forEach items="${sessionidName}" var="sessionidName">
										<li class="name">
										<input type="hidden" name="cbwarea" id="cbwarea" value="${k_homeVO.companyCode}" /> 
										<input type="hidden" name="cbreadid" id="cbreadid" value="${sessionScope.login}" /> 
										<input type="hidden" name="cbuserid" id="cbuserid" value="${sessionScope.login}" />
						...
						<div class="data_box">
							<ul class="title_box">
								...
								<li class="name"><span class="personal_img"><img
									...
									<a href="../member/individualfollow?email=${ScrollBoardVO.cbuserid}">
										<font color="RED">${ScrollBoardVO.company} (${ScrollBoardVO.first}${ScrollBoardVO.last})</font> </a> 
									</c:if>	
									<c:if test="${manager ne ScrollBoardVO.cbuserid}">
									<a href="../member/individualfollow?email=${ScrollBoardVO.cbuserid}">
										${ScrollBoardVO.company} (${ScrollBoardVO.first}${ScrollBoardVO.last}) </a> 
									</c:if>	
										<input type="hidden" id="idx${ScrollBoardVO.cbid}" value="${ScrollBoardVO.cbid}" />
									
									<c:if test="${ScrollBoardVO.cbuserid ne sessionScope.login && manager eq sessionScope.login}">
										<button class="enter" name="edit_save"
											onclick="remove(${ScrollBoardVO.cbid})">삭제</button> 
									</c:if>
									<c:if test="${ScrollBoardVO.cbuserid eq sessionScope.login && manager eq sessionScope.login}">
									<button class="enter" name="edit_save"
											onclick="remove(${ScrollBoardVO.cbid})">삭제</button> 
										<button class="enter" name="edit_save"
											onclick="mod(${ScrollBoardVO.cbid});">수정</button>
									</c:if>
									<c:if test="${ScrollBoardVO.cbuserid eq sessionScope.login && manager ne sessionScope.login }">
										<button class="enter" name="edit_save"
											onclick="remove(${ScrollBoardVO.cbid})">삭제</button> 
										<button class="enter" name="edit_save"
											onclick="mod(${ScrollBoardVO.cbid});">수정</button>
							...
							<form class="repForm" id="repForm" method="POST" action="../cboard/comreply">
								<ul class="title_box">
									...
									<c:if test="${ScrollBoardVO.cbid == replylist.comboardid}">
										<li><div>
												<span class="personal_img"><img	
													...
													<a href="../member/individualfollow?email=${replylist.comrep_userid}">
										...
										<li><div>
												<c:if test="${replylist.comrep_userid ne sessionScope.login && manager eq sessionScope.login}">
													<input type="button" value="삭제" style="margin-right: 10px"
													onclick="replyremove(${replylist.crid});">
												</c:if>
												<c:if test="${replylist.comrep_userid eq sessionScope.login && manager eq sessionScope.login}">
													<input type="button" value="수정" style="margin-right: 10px"
														onclick="replymod(${replylist.crid});">
													<input type="button" value="삭제" style="margin-right: 10px"
													onclick="replyremove(${replylist.crid});">
												</c:if>
												<c:if test="${replylist.comrep_userid eq sessionScope.login && manager ne sessionScope.login}">
													<input type="button" value="수정" style="margin-right: 10px"
														onclick="replymod(${replylist.crid});">
													<input type="button" value="삭제" style="margin-right: 10px"
													onclick="replyremove(${replylist.crid});">
