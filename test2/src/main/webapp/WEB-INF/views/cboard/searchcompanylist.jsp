...
<script type="text/javascript">
	function sendmemID(code) {
		var code = code;
		var result = confirm('등록하시겠습니까?');
		if (result) {
			$.ajax({
				url : "../cboard/searchcompanylist?code=" + code,
				type : "post",
				data : {
					code : code
				},
			                  ...
			                  if ($("#search1").val() != "") {
			                     ...
			                     var regExp =  /[ \{\}\[\]\/?.,;:|\)*~`!^\-_+┼<>\#$%&\'\"\\(\=]/gi;
			             	     if(regExp.test(str)){
			             	    	...
			             	    	document.location.href = "../cboard/searchcompanylist?keyword="+keyword;
			             	    }else{
			             	    	document.location.href = "../cboard/searchcompanylist?keyword="+str;
			             	    }
...
<body>
	...
	<section id="container">
		<c:forEach items="${companylist}" var="companylist">
			<div>
				<a
					href="javascript:sendmemID('${companylist.companyCode}','${companylist.comname_kr}','${companylist.comname_en}')">
					<input type="hidden" value="${companylist.companyCode}">
					${companylist.comname_kr}(${companylist.comname_en}) ${companylist.repname} 
					${companylist.country}
				</a>
			</div>
		</c:forEach>
